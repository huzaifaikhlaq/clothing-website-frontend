...
const Auth = () => {
    ...
    const handleSignin = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                setSigninErrors({ email: error.response?.data?.message || 'Server error. Please try again later' });
            }
        }
    };
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                setSignupErrors({ email: error.response?.data?.message || 'Server error. Please try again later' });
            }
        }
    }
    ...
    return (
        < main className="flex-grow flex flex-col items-center justify-center px-6 py-12 md:px-12 bg-[#f9f9f9]" >
                    ...
                    activeTab === 'signin' && (
                        <form className="space-y-6" onSubmit={handleSignin} noValidate>
                            <Field label="Email" error={signinErrors.email}>
                                <input className={inputCls} placeholder="email@example.com" type="email" value={signinData.email} onChange={(e) => setSigninData((prev) => ({ ...prev, email: e.target.value }))} />
                            </Field>
                            ...
                            <Field label="Password" error={signinErrors.password}>
                                ...
                                <div className="relative">
                                    <input className={inputCls} placeholder="•••••••" type={showSigninPass ? 'text' : 'password'} value={signinData.password} onChange={(e) => setSigninData((prev) => ({ ...prev, password: e.target.value }))} />
                            ...
                            <div className="pt-4 space-y-4">
                                ...
                                <button
                                    className="w-full bg-white border border-[#c6c6c6]/50 text-[#1a1c1c] py-4 font-label text-[11px] sm:text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3  md:gap-3 hover:bg-[#eeeeee] transition-colors rounded-none"
                                    type="button"
                                >
                                    <GoogleIcon />
                        ...
                        <form className="space-y-6" onSubmit={handleSignup} noValidate>
                            ...
                            <Field label="Password" error={signupErrors.password}>
                                <div className="relative">
                                    ...
                                    />
                                    <EyeBtn show={showSignupPass} onToggle={() => setShowSignupPass(!showSignupPass)} />
                                </div>
                            </Field>
                            ...
                            <Field label="Confirm Password" error={signupErrors.confirm}>
                                <div className="relative">
                                    ...
                                    />
                                    <EyeBtn show={showConfirmPass} onToggle={() => setShowConfirmPass(!showConfirmPass)} />
                                </div>
                            </Field>
            ...
            {/* Editorial Quote */}
            < div className="mt-24 text-center md:max-w-sm" >
